...
function addSecureHeader(event) {
  ...
  item.internetHeaders.setAsync({
    ...
    if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
      ...
      Office.context.mailbox.item.notificationMessages.addAsync("headerAdded", {
      ...
      // Show error
      Office.context.mailbox.item.notificationMessages.addAsync("headerError", {
        type: "errorMessage",
...
function toggleSecureHeader(event) {
  ...
  item.loadCustomPropertiesAsync(function (result) {
    ...
    var isEnabled = customProps.get("SecureHeaderEnabled") === "true";
    if (isEnabled) {
      ...
      item.internetHeaders.setAsync({
        ...
        if (asyncResult.status === Office.AsyncResultStatus.Succeeded) {
          ...
          customProps.saveAsync(function () {
            ...
            item.notificationMessages.addAsync("secureHeaderInfo", {
              type: "informationalMessage",
...
function showNotification(title, message) {
  var type = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "informationalMessage";
  Office.context.mailbox.item.notificationMessages.addAsync("headerNotification", {
    type: type,
    message: message,
    icon: "Icon.16x16",
    persistent: true
